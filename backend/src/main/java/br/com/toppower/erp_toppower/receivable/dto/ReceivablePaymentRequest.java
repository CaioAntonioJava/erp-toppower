...
/**
 * Payload para registrar um pagamento avulso contra uma conta a receber.
 * O valor abate do saldo devedor; a conta transita para PAGO
 * automaticamente quando o saldo zera.
 */
@Schema(name = "ReceivablePaymentRequest",
        description = "Dados de um pagamento avulso contra uma conta a receber.")
public record ReceivablePaymentRequest(
        ...
        @Schema(description = "Observações (forma de recebimento, comprovante, etc.).",
                example = "PIX", requiredMode = Schema.RequiredMode.NOT_REQUIRED,
                maxLength = 500)
