...
/**
 * Endereço opcional de execução da proposta técnica.
 *
 * <p>Variante <b>permissiva</b> do {@code AddressDto} comum: nenhum
 * campo é obrigatório, pois o endereço é opcional na proposta técnica.
 * Quando o objeto inteiro for nulo, nenhum endereço é persistido.</p>
 */
@Schema(name = "TechnicalProposalAddressRequest",
        description = "Endereço de execução da proposta técnica (opcional).")
public record TechnicalProposalAddressRequest(

        @Schema(description = "Logradouro.", example = "Av. Paulista", maxLength = 200,
                requiredMode = Schema.RequiredMode.NOT_REQUIRED)
        @Size(max = 200, message = "Logradouro deve ter no máximo {max} caracteres")
        String street,

        @Schema(description = "Número do imóvel. Aceita 'S/N'.", example = "1000", maxLength = 20,
                requiredMode = Schema.RequiredMode.NOT_REQUIRED)
        @Size(max = 20, message = "Número deve ter no máximo {max} caracteres")
        String number,

        @Schema(description = "Complemento (apto, bloco, sala).", example = "Apto 101", maxLength = 100,
                requiredMode = Schema.RequiredMode.NOT_REQUIRED)
