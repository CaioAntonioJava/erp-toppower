...
export function CustomersListPage() {
  ...
  const list = useEntityList<CustomerResponse>({
    api: {
      fetchAll: listCustomers,
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Cadastro e gestão de clientes pessoa física.
            {isAdmin ? (
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="grid gap-3 sm:grid-cols-[1fr_220px]">
          <Input
            placeholder="Buscar por código, nome, e-mail ou CPF…"
            value={list.query}
      ...
      {isAdmin && list.hasSelection ? (
        <div className="flex flex-col items-stretch gap-2 rounded-xl border border-primary/30 bg-primary-50 px-4 py-3 text-sm dark:border-primary-900 dark:bg-primary-900/20 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 text-primary-800 dark:text-primary-200">
            ...
            <span>
              <strong>{list.selectedIds.size}</strong> cliente(s) selecionado(s)
        ...
        <div className="flex flex-col items-center justify-between gap-3 border-t border-slate-200 px-4 py-3 text-sm sm:flex-row dark:border-slate-800">
          <span className="text-slate-500 dark:text-slate-400">
            {list.totalElements === 0
              ? 'Nenhum resultado'
              : `${list.totalElements} cliente(s) • Página ${
      ...
      <ConfirmDialog
        ...
        description={
          list.confirmSingle?.status === 'ATIVO'
            ? `O cliente "${list.confirmSingle?.name}" será marcado como inativo. O registro não é apagado e pode ser reativado depois.`
            : `O cliente "${list.confirmSingle?.name}" voltará a ficar ativo.`
        }
      ...
      <ConfirmDialog
        ...
        title={
          list.confirmBulk === 'ATIVO'
            ? 'Reativar clientes selecionados?'
            : 'Inativar clientes selecionados?'
        }
        description={
          list.confirmBulk === 'ATIVO'
            ? `${list.selectedIds.size} cliente(s) serão marcados como ativos.`
            : `${list.selectedIds.size} cliente(s) serão marcados como inativos. Os registros não são apagados.`
        }
